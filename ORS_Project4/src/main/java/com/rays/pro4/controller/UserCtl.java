...
public class UserCtl extends BaseCtl {
	...
	protected boolean validate(HttpServletRequest request) {
		System.out.println("Userctl Validate");
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (id > 0 || op != null) {
			System.out.println("In UserCtl Doget id > 0  condition");
			UserBean bean;
			try {
				bean = model.findByPK(id);
				System.out.println("UserCtl Doget After FindByPK");
				System.out.println("Bean In UserCtl DoGet = "+bean);
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, IOException {
		System.out.println("Userctl Do Post");

		log.debug("UserCtl Method doPost Started");

		String op = DataUtility.getString(request.getParameter("operation"));
		System.out.println("Operation in UserCtl DoPost = " + op);
		long id = DataUtility.getLong(request.getParameter("id"));

		System.out.println("ID in UserCtl DoPost = " + request.getParameter("id"));

		UserModel model = new UserModel();
		if (OP_SAVE.equalsIgnoreCase(op) || OP_UPDATE.equalsIgnoreCase(op)) {
			UserBean bean = (UserBean) populateBean(request);
			System.out.println("UserCtl DoPost In Update And Save Condition");
			...
			try {
				if (id > 0) {
					...
					model.update(bean);
					ServletUtility.setBean(bean, request);
					System.out.println("UserCtl Update");
					ServletUtility.setSuccessMessage("User is successfully Updated", request);

				} else {
					System.out.println("UserCtl Add or Save");
				...
				System.out.println("UserCtl DoPost DuplicateRecordException (Login id already exists)");
				ServletUtility.setBean(bean, request);
			...
			System.out.println("UserCtl DoPost Cancel Operation");
